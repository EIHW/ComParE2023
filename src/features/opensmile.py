...
audio_base='./data/wav'
output_base='./data/features/opensmile'
...
for filename in tqdm(wavs):
    cmd = f'SMILExtract -noconsoleoutput -C ./opensmile/config/{feature_set} -I {filename} -N {basename(filename)} -csvoutput {outfile} -appendcsv 1'
    os.system(cmd)
