...
def compute_metrics(devel_df, test_df, params):
    ...
    for label_type in ["true", "prediction"]:
        ...
        for i in COLUMN_NAMES:
            devel_df[f"{i}_{label_type}"] = None
        for index, row in devel_df.iterrows():
            res = devel_df[label_type][index].strip("][").split(", ")
            for res, col in zip(res, col_type):
                devel_df.at[index, col] = float(res)
        if label_type == "true":
    ...
    for label_type in ["true", "prediction"]:
        ...
        if label_type == "prediction":
            col_type = [str(i) + "_pred" for i in COLUMN_NAMES]
        for i in COLUMN_NAMES:
            test_df[f"{i}_{label_type}"] = None
        for index, row in test_df.iterrows():
            res = test_df[label_type][index].strip("][").split(", ")
            for res, col in zip(res, col_type):
                test_df.at[index, col] = float(res)
        if label_type == "true":
